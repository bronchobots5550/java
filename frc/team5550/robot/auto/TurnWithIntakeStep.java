...
public class TurnWithIntakeStep extends Step {
	...
	public TurnWithIntakeStep(AutoDriveRoutine routine, double angle) {
		super(routine);
	...
	public boolean driveTurnWheel(double degrees) {
		...
		if (Math.abs(leftDrive.getSelectedSensorPosition(0) - pos) < 200) {
			leftDrive.set(ControlMode.Current, 0);
		} else {
			leftDrive.set(ControlMode.MotionMagic, pos);
		}

		if (Math.abs(rightDrive.getSelectedSensorPosition(0) - pos) < 200) {
		...
		return (Math.abs(leftDrive.getSelectedSensorPosition(0) - pos) < 200
				&& Math.abs(rightDrive.getSelectedSensorPosition(0) + pos) < 200);
